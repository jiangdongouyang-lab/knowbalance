...
3. function 模式必须声明 entry_point；starter 和 reference 必须实现同一入口。
4. public 只包含任务说明、starter、可见测试说明、三级提示、反思问题和引用；不得出现参考解、隐藏测试、期望值、评分组或 mutation。
5. secure 必须包含 reference_solution、每个目标至少一个有区分力的 hidden test、scoring_groups、misconception_map 和逐目标覆盖映射；mutation_variants 可以为空。
6. 每个 core objective 必须同时对应至少一个 instruction block、public test、hidden test 和 scoring group。
...
9. starter 不得直接通过全部核心测试；reference 必须设计为可通过全部测试。可选 mutation 仅用于质量诊断，不影响发布。
