...
describe("Role D dynamic diagnosis and official C resources", () => {
  ...
  test("renders official C artifacts and keeps grading pending", async () => {
    ...
    expect(screen.queryByText(/Tier [23]/)).not.toBeInTheDocument()
    expect(screen.queryByText("补全 average_score。")).not.toBeInTheDocument()
  ...
  test("captures every public C response type and submits locally", async () => {
    ...
    const route = screen.getByRole("button", { name: "确认锚点并继续" })
    ...
    await screen.findByLabelText("第 3 题代码追踪答案")
    expect(screen.getByRole("button", { name: "A. 依次处理列表中的每个元素" })).toBeDisabled()
    expect(screen.getByRole("button", { name: "B. 正确" })).toBeDisabled()
    ...
    const submit = screen.getByRole("button", { name: "提交本轮测评" })
