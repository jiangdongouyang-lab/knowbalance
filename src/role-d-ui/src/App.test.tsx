...
describe("Role D local users and learning plans", () => {
  ...
  test("keeps multiple plans resumable and isolates them by local user", async () => {
    ...
    expect(screen.getByRole("heading", { name: "创建本机学习档案" })).toBeInTheDocument()
    expect(screen.getByLabelText("怎么称呼你 *")).toBeVisible()
    expect(screen.getByLabelText("专业、年级或职业")).toBeVisible()
    expect(screen.getByLabelText("每周可学习时间")).toBeVisible()
    expect(screen.getByLabelText("接触过的编程语言")).toBeVisible()
