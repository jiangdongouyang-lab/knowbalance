...
import { runDynamicPlanningLoop } from "../../orchestration/dynamic-planning-orchestrator"
...
export function App() {
  ...
  const continueToNextStage = async () => {
    ...
    setOnboardingErrors((current) => ({ ...current, [targetPlanId]: "" }))
    try {
      const loop = await runDynamicPlanningLoop({
        initial_submission: {
        ...
        max_rounds: 1,
        continue_after_submission: continueRoleCAfterSubmission,
      })
      const result = loop.latest_result
      if (loop.status === "awaiting_submission" && result?.status === "published") {
      ...
      } else if (loop.status === "awaiting_input" && result?.status === "awaiting_input") {
        ...
        const reason = result && "reason" in result ? result.reason : loop.final_round?.reason
        setOnboardingErrors((current) => ({ ...current, [targetPlanId]: reason ?? "C 未能完成下一阶段准备。" }))
      }
    } catch (error) {
