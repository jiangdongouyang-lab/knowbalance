...
export function DiagnosisScreen({ diagnosis, answers, submitted, submitting, onAnswer, onSubmit, onContinue, onBack }: DiagnosisScreenProps) {
  ...
  return (
    <section className="stage-screen diagnosis-screen">
      <div className="screen-heading"><span className="screen-step">学习流程 2 / 6 · 客观诊断</span><h1>用真实知识库题目确认基础</h1><p>系统只展示 A 当前命中的真实选择题；精确目标不会额外借题或临时编造。有多少可追溯题就展示多少，最多 5 道；具体来源可在“知识证据”中查看。</p></div>
      {unavailable
      ...
      {!submitted && !unavailable && <p className="diagnosis-progress" role="status">已答 {answeredCount} / {items.length} 题 · {answeredCount === items.length ? "可以提交诊断" : "请完成全部真实题后提交"}</p>}
