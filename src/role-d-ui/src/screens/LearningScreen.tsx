...
export function LearningScreen({ session, onTab, onCitation, onAssessmentAnswer, onAssessmentSubmit, onContinue, onBack }: LearningScreenProps) {
  ...
  return (
    <section className="stage-screen learning-screen">
      ...
      <LearningWorkspace artifacts={session.artifacts} activeKind={session.view.activeArtifactKind} assessmentAnswers={session.view.assessmentAnswers ?? {}} assessmentSubmitted={session.view.assessmentSubmitted === true} assessmentStatus={session.view.assessmentStatus} assessmentMessage={session.view.assessmentMessage} onKindChange={onTab} onCitationSelect={onCitation} onAssessmentAnswer={onAssessmentAnswer} onAssessmentSubmit={onAssessmentSubmit} />
